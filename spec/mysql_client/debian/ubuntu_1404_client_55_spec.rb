...
describe 'mysql_client_test::default on ubuntu-14.04' do
  cached(:ubuntu_1404_client_55) do
  ...
  context 'compiling the test recipe' do
    it 'creates mysql_client[default]' do
      expect(ubuntu_1404_client_55).to create_mysql_client('default')
    end
  end
  ...
  context 'stepping into mysql_client[default] resource' do
    ...
    it 'installs package[default :create libmysqlclient-dev]' do
      expect(ubuntu_1404_client_55).to install_package('default :create libmysqlclient-dev')
