...
describe 'mysql_service_test::single on ubuntu-14.04' do
  ...
  context 'compiling the test recipe' do
    it 'creates mysql_service[default]' do
      expect(ubuntu_1404_service_56_single).to create_mysql_service('default')
    end
  end

  # mysql_service resource internal implementation
  context 'stepping into mysql_service[default] resource' do
    it 'installs package[default :create mysql-server-5.6]' do
      expect(ubuntu_1404_service_56_single).to install_package('default :create mysql-server-5.6')
        .with(package_name: 'mysql-server-5.6', version: nil)
    end

    it 'stops service[default :create mysql]' do
      expect(ubuntu_1404_service_56_single).to disable_service('default :create mysql')
      expect(ubuntu_1404_service_56_single).to stop_service('default :create mysql')
    end

    it 'installs package[default :create apparmor]' do
      expect(ubuntu_1404_service_56_single).to install_package('default :create apparmor')
    ...
    it 'creates service[default :create apparmor]' do
      expect(ubuntu_1404_service_56_single).to_not start_service('default :create apparmor').with(
        service_name: 'apparmor'
        )
    end

    it 'creates group[default :create mysql]' do
      expect(ubuntu_1404_service_56_single).to create_group('default :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[default :create mysql]' do
      expect(ubuntu_1404_service_56_single).to create_user('default :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[default :create /etc/mysql/my.cnf]' do
      expect(ubuntu_1404_service_56_single).to delete_file('default :create /etc/mysql/my.cnf')
        .with(path: '/etc/mysql/my.cnf')
    end

    it 'deletes file[default :create /etc/my.cnf]' do
      expect(ubuntu_1404_service_56_single).to delete_file('default :create /etc/my.cnf')
    ...
    it 'runs bash[default :create initial records]' do
      expect(ubuntu_1404_service_56_single).to_not run_bash('default :create initial records')
    end
