...
describe 'mysql_service_test::single on debian-7.0' do
  cached(:debian_70_service_55_multi) do
  ...
  context 'compiling the test recipe' do
    it 'creates user[alice]' do
      expect(debian_70_service_55_multi).to create_user('alice')
    end

    it 'creates group[alice]' do
      expect(debian_70_service_55_multi).to create_group('alice')
    end

    it 'creates user[bob]' do
      expect(debian_70_service_55_multi).to create_user('bob')
    end

    it 'creates group[bob]' do
      expect(debian_70_service_55_multi).to create_group('bob')
    end

    it 'deletes mysql_service[default]' do
      expect(debian_70_service_55_multi).to delete_mysql_service('default')
    end
  ...
  context 'stepping into mysql_service[default] resource' do
    it 'stops service[default :delete mysql-default]' do
      expect(debian_70_service_55_multi).to_not disable_service('default :delete mysql-default')
      expect(debian_70_service_55_multi).to_not stop_service('default :delete mysql-default')
    end

    it 'deletes directory[default :delete /etc/mysql-default]' do
      expect(debian_70_service_55_multi).to delete_directory('default :delete /etc/mysql-default')
    end

    it 'deletes directory[default :delete /run/mysql-default]' do
      expect(debian_70_service_55_multi).to delete_directory('default :delete /run/mysql-default')
    end

    it 'deletes directory[default :delete /var/log/mysql-default]' do
      expect(debian_70_service_55_multi).to delete_directory('default :delete /var/log/mysql-default')
    end
  end
  ...
  context 'stepping into mysql_service[instance-1] resource' do
    it 'installs package[instance-1 :create mysql-server-5.5]' do
      expect(debian_70_service_55_multi).to install_package('instance-1 :create mysql-server-5.5')
        .with(package_name: 'mysql-server-5.5', version: nil)
    end

    it 'stops service[instance-1 :create mysql]' do
      expect(debian_70_service_55_multi).to disable_service('instance-1 :create mysql')
      expect(debian_70_service_55_multi).to stop_service('instance-1 :create mysql')
    end

    it 'creates group[instance-1 :create mysql]' do
      expect(debian_70_service_55_multi).to create_group('instance-1 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-1 :create mysql]' do
      expect(debian_70_service_55_multi).to create_user('instance-1 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-1 :create /etc/mysql/my.cnf]' do
      expect(debian_70_service_55_multi).to delete_file('instance-1 :create /etc/mysql/my.cnf')
        .with(path: '/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-1 :create /etc/my.cnf]' do
      expect(debian_70_service_55_multi).to delete_file('instance-1 :create /etc/my.cnf')
    ...
    it 'runs bash[instance-1 :create initial records]' do
      expect(debian_70_service_55_multi).to_not run_bash('instance-1 :create initial records')
    end
  ...
  context 'stepping into mysql_service[instance-2] resource' do
    it 'installs package[instance-2 :create mysql-server-5.5]' do
      expect(debian_70_service_55_multi).to install_package('instance-2 :create mysql-server-5.5')
        .with(package_name: 'mysql-server-5.5', version: nil)
    end

    it 'stops service[instance-2 :create mysql]' do
      expect(debian_70_service_55_multi).to disable_service('instance-2 :create mysql')
      expect(debian_70_service_55_multi).to stop_service('instance-2 :create mysql')
    end

    it 'creates group[instance-2 :create mysql]' do
      expect(debian_70_service_55_multi).to create_group('instance-2 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-2 :create mysql]' do
      expect(debian_70_service_55_multi).to create_user('instance-2 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-2 :create /etc/mysql/my.cnf]' do
      expect(debian_70_service_55_multi).to delete_file('instance-2 :create /etc/mysql/my.cnf')
        .with(path: '/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-2 :create /etc/my.cnf]' do
      expect(debian_70_service_55_multi).to delete_file('instance-2 :create /etc/my.cnf')
        .with(path: '/etc/my.cnf')
    end
    ...
    it 'runs bash[instance-2 :create initial records]' do
      expect(debian_70_service_55_multi).to_not run_bash('instance-2 :create initial records')
    end
