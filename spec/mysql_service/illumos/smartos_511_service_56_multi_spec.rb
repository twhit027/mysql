...
describe 'mysql_service_test::multi on smartos-5.11' do
  cached(:smartos_511_service_56_multi) do
  ...
  context 'compiling the test recipe' do
    it 'creates user[alice]' do
      expect(smartos_511_service_56_multi).to create_user('alice')
    end

    it 'creates group[alice]' do
      expect(smartos_511_service_56_multi).to create_group('alice')
    end

    it 'creates user[bob]' do
      expect(smartos_511_service_56_multi).to create_user('bob')
    end

    it 'creates group[bob]' do
      expect(smartos_511_service_56_multi).to create_group('bob')
    end

    it 'deletes mysql_service[default]' do
      expect(smartos_511_service_56_multi).to delete_mysql_service('default')
    end

    it 'creates mysql_service[instance-1]' do
      expect(smartos_511_service_56_multi).to create_mysql_service('instance-1')
    end

    it 'creates mysql_service[instance-2]' do
      expect(smartos_511_service_56_multi).to create_mysql_service('instance-2')
    end
  end
  ...
  context 'stepping into mysql_service[instance-1] resource' do
    ...
    it 'creates group[instance-1 :create mysql]' do
      expect(smartos_511_service_56_multi).to create_group('instance-1 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-1 :create mysql]' do
      expect(smartos_511_service_56_multi).to create_user('instance-1 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-1 :create /opt/local/etc/mysql/my.cnf]' do
      expect(smartos_511_service_56_multi).to delete_file('instance-1 :create /opt/local/etc/mysql/my.cnf')
        .with(path: '/opt/local/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-1 :create /opt/local/etc/my.cnf]' do
      expect(smartos_511_service_56_multi).to delete_file('instance-1 :create /opt/local/etc/my.cnf')
    ...
    it 'runs bash[instance-1 :create initial records]' do
      expect(smartos_511_service_56_multi).to_not run_bash('instance-1 :create initial records')
    end
  ...
  context 'stepping into mysql_service[instance-2] resource' do
    ...
    it 'creates group[instance-2 :create mysql]' do
      expect(smartos_511_service_56_multi).to create_group('instance-2 :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[instance-2 :create mysql]' do
      expect(smartos_511_service_56_multi).to create_user('instance-2 :create mysql')
        .with(username: 'mysql')
    end

    it 'deletes file[instance-2 :create /opt/local/etc/mysql/my.cnf]' do
      expect(smartos_511_service_56_multi).to delete_file('instance-2 :create /opt/local/etc/mysql/my.cnf')
        .with(path: '/opt/local/etc/mysql/my.cnf')
    end

    it 'deletes file[instance-2 :create /opt/local/etc/my.cnf]' do
      expect(smartos_511_service_56_multi).to delete_file('instance-2 :create /opt/local/etc/my.cnf')
        .with(path: '/opt/local/etc/my.cnf')
    end
    ...
    it 'runs bash[instance-2 :create initial records]' do
      expect(smartos_511_service_56_multi).to_not run_bash('instance-2 :create initial records')
    end
