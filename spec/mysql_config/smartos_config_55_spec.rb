...
describe 'mysql_config_test::default' do
  ...
  context 'compiling the test recipe' do
    it 'creates mysql_config[hello]' do
      expect(smartos_config_55).to create_mysql_config('hello')
    end

    it 'creates mysql_config[hello_again]' do
      expect(smartos_config_55).to create_mysql_config('hello_again')
    end
  end
  ...
  context 'stepping into mysql_config[hello] resource' do
    it 'creates group[hello :create mysql]' do
      expect(smartos_config_55).to create_group('hello :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[hello :create mysql]' do
      expect(smartos_config_55).to create_user('hello :create mysql')
        .with(username: 'mysql')
    end
    ...
    it 'creates group[hello_again :create mysql]' do
      expect(smartos_config_55).to create_group('hello_again :create mysql')
        .with(group_name: 'mysql')
    end

    it 'creates user[hello_again :create mysql]' do
      expect(smartos_config_55).to create_user('hello_again :create mysql')
        .with(username: 'mysql')
    end
