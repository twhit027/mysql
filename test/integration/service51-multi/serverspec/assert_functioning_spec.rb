...
def mysqld_bin
  ...
  return '/opt/rh/mysql51/root/usr/libexec/mysqld' if os[:family] =~ /redhat/ && os[:release] =~ /5\./
  return '/usr/libexec/mysqld' if os[:family] =~ /redhat/ && os[:release] =~ /6\./
  '/usr/sbin/mysqld'
end
...
describe command(mysqld_cmd) do
  its(:exit_status) { should eq 0 }
  its(:stdout) { should match(/Ver 5.1/) }
end
